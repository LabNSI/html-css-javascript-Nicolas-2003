function init () {
  ...
  // sReq = //programme.html?prenom=Nicolas&nom=Reche&classe=1G2&MATHEMATIQUES=MATHEMATIQUES&NUMERIQUE_SC_INFORM=NUMERIQUE+SC.INFORM
...
function resultat() {
  ...
  // Vérifier si la classe a été coché. Si non (chaine vide), ajouter "\n- Classe" à message
  ...
  // Vérifier si deux spécialité ont été choisi. Si non (chaine vide), ajouter "\n- Deux spécialités" 
  if(cpt!=2){
    message += "\n- Deux spécialités";
  }

  // Vérifier si les champs ont été complété. Si non (chaine vide), faire apparaitre message en alert
  ...
  // Vérifier si "NUMERIQUE_SC_INFORM" a été coché. Si oui, ajouter "\nTu fais le bon choix !". Si non, ajouter "\nEs-tu sur de ton choix ? !"
